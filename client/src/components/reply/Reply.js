...
function Reply ({comment, axiosInstance, clickedInfo, post, setPosts, userInfo}) {
    ...
    const handleUpdateReply = async () => {
        ...
        await axiosInstance.put(`/posts/${post.id}/comment`, {
            text }, {params: {commentId: `${comment._id}`}
        });
            ...
            <div>
                ...
                {isOpenReply === true ?
                    ...
                    <input className={styles.modifyInput}
                            ...
                            ref={inputRef}
                    />
                    <button className={styles.cancle} onClick={handleCloseReply}>취소</button>
                    <button className={styles.modifyConfirm} onClick={handleUpdateReply}>수정하기</button>
                    </span>
