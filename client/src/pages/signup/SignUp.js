...
const onChangePassword = useCallback((e) => {
  ...
  if(!passwordPattern.test(passwordCurrent)){
    setPasswordMessage('비밀번호는 영문 대소문자 특수문자를 포함 8글자 이상이여야합니다')
  ...
  const onChangePasswordConfirm = useCallback((e) =>{
    ...
    if(password === passwordConfirmPattern) {
      ...
      setConfirmPasswordMessage('비밀번호가 일치하지 않습니다. 다시 확인해주세요')
          ...
          <input className={styles.userinfo}
            ...
            <div className={styles.formbox}>
              {passwordConfirm.length > 0 && (
                <span className={`message ${valConfirmPassword ? "success ": "error"}`}>
